...
class MLMF(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.audio_factor = Parameter(torch.Tensor(
            self.rank, self.audio_hidden + 1, self.output_dim))
        self.video_factor = Parameter(torch.Tensor(
            self.rank, self.video_hidden + 1, self.output_dim))
        self.text_factor = Parameter(torch.Tensor(
            self.rank, self.text_out + 1, self.output_dim))
        ...
        self.fusion_bias = Parameter(torch.Tensor(1, args.num_classes))
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        add_one = torch.ones(size=[batch_size, 1], requires_grad=False).type_as(
            audio_h).to(text_x.device)
        ...
        fusion = torch.matmul(self.fusion_weights, fusion_zy.permute(
