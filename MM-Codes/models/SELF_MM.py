...
class SELF_MM(nn.Module):
    def __init__(self, args):
        ...
        self.post_fusion_layer_3 = nn.Linear(args.post_fusion_dim, args.num_classes)
        ...
        self.post_text_layer_3 = nn.Linear(args.post_text_dim, args.num_classes)
        ...
        self.post_audio_layer_3 = nn.Linear(args.post_audio_dim, args.num_classes)
        # test
        self.audio_classifier = nn.Linear(args.audio_out, args.num_classes)
        ...
        self.post_video_layer_3 = nn.Linear(args.post_video_dim, args.num_classes)
        # test
        self.video_classifier = nn.Linear(args.video_out, args.num_classes)

    def __tanD(self, x):
        x_shape = x.size()
        x = x.reshape(-1)
        left = x < -self.D
        middle = (x >= -self.D) & (x <= self.D)
        right = x > self.D
        tmp_x = x.clone()
        # x < -self.D
        tmp_x[left] = (1 - torch.exp(-x[left]-self.D)) / (1 + torch.exp(-x[left]-self.D))
        # x < self.D and x > -self.D
        tmp_x[middle] = 0
        # x > self.D
        tmp_x[right] = (1 - torch.exp(-x[right]+self.D)) / (1 + torch.exp(-x[right]+self.D))
        # return
