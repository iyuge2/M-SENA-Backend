...
class EF_LSTM(nn.Module):
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        fusion_h = final_states[0][-1].squeeze()
        x = self.dropout(fusion_h)
        x = F.relu(self.linear(x), inplace=True)
        x = self.dropout(x)
        output = self.out(x)
        res = {
            'Feature_T': text_x.mean(dim=1),
            'Feature_A': audio_x.mean(dim=1),
            'Feature_V': video_x.mean(dim=1),
            'Feature_M': fusion_h,
            'M': output
        }
        return res 
